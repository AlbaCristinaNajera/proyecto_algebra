...
// Función para resolver con la Regla de Cramer
function solveWithCramer() {
    ...
    if (n === 2) {
        ...
        if (delta === 0) {
            ...
            const x1 = (b[0] * a[1][1] - a[0][1] * b[1]) /delta;
            const x2 = (a[0][0] * b[1] - b[0] * a[1][0]) / delta;
            result.innerHTML = `Resultado de Cramer: x1 = ${x1}, x2 = ${x2}`;
        }
    } else if (n === 3) {
            // Sistema 3x3
        const delta = 
        ...
        if (delta === 0) {
            ...
            const x1 = (b[0] * (a[1][1] * a[2][2] - a[1][2] * a[2][1]) - a[0][1] * (b[1] * a[2][2] - a[1][2] * b[2]) + a[0][2] * (b[1] * a[2][1] - a[1][1] * b[2])) / delta;
            const x2 = (a[0][0] * (b[1] * a[2][2] - a[1][2] * b[2]) - b[0] * (a[1][0] * a[2][2] - a[1][2] * a[2][0]) + a[0][2] * (a[1][0] * b[2] - b[1] * a[2][0])) / delta;
            const x3 = (a[0][0] * (a[1][1] * b[2] - b[1] * a[2][1]) - a[0][1] * (a[1][0] * b[2] - b[1] * a[2][0]) + b[0] * (a[1][0] * a[2][1] - a[1][1] * a[2][0])) / delta;
        
            result.innerHTML = `Resultado de Cramer: x1 = ${x1}, x2 = ${x2}, x3 = ${x3}`;
            }
        }
...
function solveWithGaussJordan() {
    ...
    if (n === 2) {
        ...
        const x1 = matrix[0][cols - 1];
        const x2 = matrix[1][cols - 1];

        result.innerHTML = `Resultado de Gauss-Jordan: x1 = ${x1}, x2 = ${x2}`;
        ...
        const x1 = matrix[0][cols - 1];
        const x2 = matrix[1][cols - 1];
        const x3 = matrix[2][cols - 1];

        result.innerHTML = `Resultado de Gauss-Jordan: x1 = ${x1}, x2 = ${x2}, x3 = ${x3}`;
    }
}
